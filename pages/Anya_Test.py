...
# ── Rich Styles 預覽：各 Tab 的 markdown 內容（供 copy_html_button 取用）──
_MD_MARKET = """
...
> **風險提示**：以上為示範數據，不構成投資建議。實際交易前請參閱最新市場資訊。
"""
...
> **選型原則**：先看資料型態（表格 / 影像 / 文字），再看資料量，再看可解釋性需求。
...
> **翻成白話**：這不是「空氣炒作」，而是「有真東西，但市場可能先把未來很多年都提前反映了」。
...
with st.expander(":material/palette: Rich Styles 預覽", expanded=False):
    ...
    with _tab_market:
        ...
        copy_html_button(_MD_MARKET, key="copy_market")

    # ── Tab 2：技術比較（h2/h3 層級 + inline code + 有序清單）
    with _tab_tech:
        st.markdown(_MD_TECH)
        copy_html_button(_MD_TECH, key="copy_tech")
    ...
    with _tab_report:
        ...
        copy_html_button(_MD_REPORT, key="copy_report")

    # ── Tab 4：對話回答（一般 Q&A 風格，無 report header）
    with _tab_qa:
        st.markdown(_MD_QA)
        copy_html_button(_MD_QA, key="copy_qa")
...
- emoji直接輸入如😄，Material Symbols如:material/star:，用法見[Google Material Symbols](https://fonts.google.com/icons)。

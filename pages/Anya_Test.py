...
CRITIQUE_ANALYSIS_TOOL = {
    ...
    "description": (
        "對分析型報告草稿執行五維度缺口驗證（批判性視角 / 條件性結論 / 方法論透明度 / 反向解讀 / 論證邏輯有效性）。"
...
2. **範圍 constraint 檢查**：使用者明示或隱含的限制（品牌、系列、語言、數量、地區、時區等）必須遵守；
   不確定某品項／機構／數據是否在限定範圍內 → 寧可不推、不可編造；不確定就老實說「不確定」。
...
- **Markdown 清單格式**：需要巢狀清單時，必須使用真正的縮排語法（子項目前加 2–4 個空白），不得用粗體文字模擬層級。範例：
  ```
  - 第一層
    - 第二層（2 spaces）
      - 第三層（4 spaces）

...
# Markdown渲染規範
你輸出的內容會直接用 Streamlit 的 st.markdown() 渲染，請嚴格遵守 Markdown 粗體規則，否則會顯示失敗：
【粗體（Bold）硬性規則】
1) 粗體只能用：**文字**
2) 在兩個星號的「內側」禁止出現任何空白字元（含看不見的空白）：
   - 禁止：** 文字**、**文字 **、** 文字 **、**\\u00A0文字**（NBSP）、**\\u3000文字**（全形空白）、**\\u200B文字**（zero-width space）
   - 允許：**雲端 AI / 高效能運算（HPC）**、**先進封裝（如 CoWoS）**
3) 星號「外側」可有空白或標點，例如：產業趨勢：**雲端 AI** 帶來機會。

【輸出前自我檢查（必須執行）】
- 在輸出最終答案前，請掃描全文並保證：
  - 不得出現字面上的「** 」(兩個星號後面立刻接空白)
  - 不得出現字面上的「 **」(空白後面立刻接兩個星號)
- 若出現上述情況，你必須先把星號內側的空白移除再輸出。
  例：** TL;DR: ** → **TL;DR:**

【額外限制】
- 不要在 code block（```...```）或 inline code（`...`）裡套用粗體標記。
- 若你不確定某段是否會引入不可見空白，寧可不要加粗體（改寫句子或移除粗體），以確保渲染正確。

請直接輸出內容，不要解釋規則、不輸出檢查過程。

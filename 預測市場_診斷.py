...
def probe_fees_and_new_fields(gamma: str, clob: str, sample_n: int) -> Probe:
    ...
    # ── oneMonthPriceChange 單位判定 ──
    # 主判準用值域，不用比值：價格在 0–1 之間，所以一個月的價格差**不可能**超過 1.0。
    # 只要出現任何 |值| > 1.0，就證明它是百分點而不是價格差——這是硬邏輯，不是啟發式。
    omc_vals = [f(m.get("oneMonthPriceChange")) for m in mkts]
    omc_vals = [x for x in omc_vals if x is not None]
    over_one = [x for x in omc_vals if abs(x) > 1.0]
    mx = max((abs(x) for x in omc_vals), default=None)

    if not omc_vals:
        unit_verdict = "無法判定：取樣中沒有 oneMonthPriceChange"
    elif over_one:
        unit_verdict = f"**是百分點**：有 {len(over_one)} 個值的絕對值 > 1.0（價格差不可能超過 1）——主頁的 ×100 要拿掉"
    else:
        unit_verdict = f"**是 0–1 價格差**：所有值都 ≤ 1.0（最大 {mx}）——主頁的 ×100 正確"

    # ── 輔助：拿真的有動過的 market 做比值對帳 ──
    # 分母太小會讓比值爆炸，所以只挑一個月確實移動 ≥ 2pp 的；試最多 5 個。
    ratio_check: dict = {"狀態": "取樣中找不到一個月移動 ≥ 2pp 的 market，無法做比值對帳（不代表單位有問題）"}
    tried = 0
    for m in mkts:
        if tried >= 5:
            break
        ...
        hist = call_json(p, f"{clob}/prices-history",
                         ...
                         f"對帳 oneMonthPriceChange #{tried}")
        ...
        if abs(actual) < 0.02:           # 漲跌相抵 → 分母太小，比值沒有意義
            continue
        ratio = omc / actual
        ratio_check = {
            ...
            "兩者比值": round(ratio, 3),
            "判讀": ("與走勢一致（同一個單位）" if 0.5 <= abs(ratio) <= 2
                     else f"與走勢差 {abs(ratio):.0f} 倍——值域判定與此矛盾，兩個都要人工看"),
        }
        break
    ...
    if over_one:
        verdicts.append("⚠️ oneMonthPriceChange 是百分點不是價格差，主頁的月變化欄位現在差 100 倍")
        p.verdict = "FAIL"
    elif not omc_vals:
        verdicts.append("oneMonthPriceChange 在取樣中不存在，月變化欄位會整欄空白")
        p.verdict = "WARN"

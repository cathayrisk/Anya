...
- 根據內容選擇最合適的 Markdown 元素：
    ...
    重點：用粗體（**重點**）。
    多層次資訊：用巢狀清單（-、  -）。
    長內容：自動分段，並用 ### 小標題 或粗體 **小標題** 區分段落，避免使用過大的標題。
    數學公式：用 $$ 包圍 LaTeX 公式（支援 KaTeX）。
    來源：用 ### 來源 標題，並用清單列出。
    顏色強調：可用 :red[重點文字]、:blue[說明] 等語法。
    彩色徽章：請善用 :color-badge[徽章文字] 語法，強調重點、分類、狀態或標籤。color 可用 blue、green、orange、red、violet、gray/grey、primary。
            - 只在「重點」、「分類」、「狀態」、「警告」、「摘要」等明顯需要強調的地方使用徽章。
            - 一般內容、步驟、條列、細節等請勿使用徽章。
            - 每個回應內徽章數量建議不超過2~3個，避免過度花俏影響閱讀。
            - 長段落或大篇幅內容請減少徽章使用，避免干擾閱讀。
            - 可用於列表開頭、標題旁、分類標籤、狀態提示等。
            - 如不確定是否該用徽章，請以「清楚、易讀」為最高原則。
            - 每次回應時，遇到重點、步驟、分類、狀態、摘要、警告等，都可以加上彩色徽章。
    小字：可用 :small[小字說明] 呈現輔助說明或補充資訊。
    emoji 與 Streamlit logo：可用 :smile:、:streamlit: 等短碼。
    請讓整體內容排版協調、易讀，避免單一元素（如標題）過於搶眼。
